...
| Component | Purpose | OpenDT Implementation |
|-----------|---------|----------------------|
| Physical Infrastructure (P) | Data source | dc-mock service |
| Front-end (B) | User interfaces | Grafana + REST API |
| Orchestration (C) | Message routing | Kafka |
| Data Platform (D, E) | Telemetry & storage | Parquet files |
| Simulator (F-I) | Power prediction | OpenDC + calibrator |
| Virtual Infrastructure (V) | Digital twin state | Topology configuration |

See the [Architecture](/docs/concepts/architecture) page for detailed component mapping.
